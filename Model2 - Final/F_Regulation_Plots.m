...
ylabel({'Tp5_{1} Degradation'; 'Inhibition Strength (A.U.)'},'fontsize',18,...
    'FontName','Arial');
title('{\it R_{2,2}(t)} ({\itK_{m,3}})','FontName','Arial','FontSize',18,'FontWeight','normal');
axis([0 96 0 1]);
xticks([0:24:96]);
...
Km4 = mpars(8);
for i = 1:length(vDNAin0)
    eq = Km4./(Km4 + vDNAtots{1,i});
    plot(tspan,eq,cols(i),'LineWidth',2); hold on;
end
xlabel('Time (hpi)','fontsize',18,'FontName','Arial');
ylabel({'Tp5_{2} Degradation'; 'Inhibition Strength (A.U.)'},'fontsize',18,...
    'FontName','Arial');
title('{\it R_{2,2}(t)} ({\itK_{m,4}})','FontName','Arial','FontSize',18,'FontWeight','normal');
axis([0 96 0 1]);
xticks([0:24:96]);
set(gca,'fontsize',16,'FontName','Arial','LineWidth',1);
box off;