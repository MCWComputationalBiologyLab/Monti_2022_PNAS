%% Error for stats. DO NOT use for par estimation
function varargout = Error_Stats(mpar,Data1,Data2,Data3,vDNAin0s,GMaxes)
...
for m = 1:length(Tp5_1Data(1,:))
    % Isolating normalized data (Data/GMax_Data)
    Tp5_1Data_curr = Tp5_1Data(:,m);
    Tp5_2Data_curr = Tp5_2Data(:,m);
    VirusData_curr = VirusData(:,m);
    
    % Isolating normalized model (Model/GMax_Model)
    Tp5_1Model_curr = (SumP1s{2,m}/GMax_SP1)';
    Tp5_2Model_curr = (SumP2s{2,m}/GMax_SP2)';
    VirusModel_curr = (Virus_Models{2,m}/GMax_Virus)';
    
    Resids_Tp5_1 = Tp5_1Model_curr - Tp5_1Data_curr;
    Resids_Tp5_2 = Tp5_2Model_curr - Tp5_2Data_curr;
    Resids_Virus = VirusModel_curr - VirusData_curr;
    
    Resids_curr = [Resids_Tp5_1;Resids_Tp5_2;Resids_Virus];
    Resids = [Resids;Resids_curr];
end

if (nargout == 1)
    varargout = {Err};
elseif (nargout == 2)
    varargout = {Err,Resids};
else
    varargout = {Err,Resids,Tp5_1Models,Tp5_2Models,Virus_Models};
end
