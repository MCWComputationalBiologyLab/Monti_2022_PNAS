...
% Formatting
colorbar;
xtl = ["k_{s,1}";"K_{m,1}";"k_{d,1}";"K_{m,3}";"k_{s,2}";"K_{m,2}";...
    "k_{d,2}";"K_{m,4}";"k_{s,C}";"k_{s,P}";"k_{ex}"];
xt = [1:1:length(mpars)];
ytl = ["k_{s,1}";"K_{m,1}";"k_{d,1}";"K_{m,3}";"k_{s,2}";"K_{m,2}";...
    "k_{d,2}";"K_{m,4}";"k_{s,C}";"k_{s,P}";"k_{ex}"];
yt = [1:1:length(mpars)];
...
names = ["k_{s,1}";"K_{m,1}";"k_{d,1}";"K_{m,3}";"k_{s,2}";"K_{m,2}";...
    "k_{d,2}";"K_{m,4}";"k_{s,C}";"k_{s,P}";"k_{ex}"];
cols = ["-k","-k","-k","-b","-b","-r","-b","-m","-k","-b","-r"];
for i = 1:length(Errs)
   xs_curr = Errs{1,i};
...
subplot(2,2,1);
legend(names(1),names(5),'FontName','Arial','FontSize',14,'Location',...
    'North','box','off');
...
subplot(2,2,2);
legend(names(2),names(4),names(6),names(8),...
...
subplot(2,2,3);
legend(names(3),names(7),'FontName','Arial','FontSize',14,'Location',...
    'North','box','off');
axis([0.5 1.5 0.75 2.75]);
set(gca, 'FontName','Arial','FontSize',14,'box','off','XTick',...
    (0.5:0.25:1.5),'YTick',(0.75:0.5:2.75),'LineWidth',1.5);
...
subplot(2,2,4);
legend(names(9),names(10),names(11),'FontName','Arial','FontSize',14,...
