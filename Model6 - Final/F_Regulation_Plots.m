...
ylabel({'Tp5_{1} Feedback';'Inhibition Strength (A.U.)'},'fontsize',18,...
    'FontName','Arial');
title('{\it R_{1,6}(t)} ({\itK_{m,3}})','FontName','Arial','FontSize',18,'FontWeight','normal');
axis([0 96 0 1]);
xticks([0:24:96]);
set(gca,'fontsize',16,'FontName','Arial','LineWidth',1);
box off;

% Km5 positive regulation
subplot(2,2,2);
Km5 = mpars(5);
for i = 1:length(vDNAin0)
    eq = vDNAtots{1,i}./(Km5 + vDNAtots{1,i});
    plot(tspan,eq,cols(i),'LineWidth',2); hold on;
end
xlabel('Time (hpi)','fontsize',18,'FontName','Arial');
ylabel({'Tp5_{1} Degradation';'Activation Strength (A.U.)'},'fontsize',18,...
    'FontName','Arial');
title('{\it R_{2,6}(t)} ({\itK_{m,5}})','FontName','Arial','FontSize',18,'FontWeight','normal');
axis([0 96 0 1]);
xticks([0:24:96]);
set(gca,'fontsize',16,'FontName','Arial','LineWidth',1);
box off;
...
ylabel({'Tp5_{2} Feedback'; 'Inhibition Strength (A.U.)'},'fontsize',18,...
    'FontName','Arial');
title('{\it R_{1,6}(t)} ({\itK_{m,4}})','FontName','Arial','FontSize',18,'FontWeight','normal');
axis([0 96 0 1]);
xticks([0:24:96]);
set(gca,'fontsize',16,'FontName','Arial','LineWidth',1);
box off;

% Km6 positive regulation
subplot(2,2,4);
Km6 = mpars(10);
for i = 1:length(vDNAin0)
    eq = vDNAtots{1,i}./(Km6 + vDNAtots{1,i});
    plot(tspan,eq,cols(i),'LineWidth',2); hold on;
end
xlabel('Time (hpi)','fontsize',18,'FontName','Arial');
ylabel({'Tp5_{2} Degradation';'Activation Strength (A.U.)'},'fontsize',18,...
    'FontName','Arial');
title('{\it R_{2,6}(t)} ({\itK_{m,6}})','FontName','Arial','FontSize',18,'FontWeight','normal');
axis([0 96 0 1]);
xticks([0:24:96]);
set(gca,'fontsize',16,'FontName','Arial','LineWidth',1);
box off;